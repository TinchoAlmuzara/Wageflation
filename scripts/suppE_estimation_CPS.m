...
% 
% The script estimates the model in different data cuts (industry average,
% and industry unweighted) and produces figures with trend estimates and 
% decompositions.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
data_cuts = {'industries_average', 'industries_unweighted'};
...
if (estimation == true)
    ...
    % Compute wage volatility
    TWV_c_draws = sqrt( squeeze( sum(repmat(share, 1, 1, n_draw) .* output_TWIn.alpha_eps, 2).^2 ) .* (output_TWIn.sigma_eps_c.^2) );
    theta_draws  = permute(repmat(ones(n, n_draw) + squeeze(sum(output_TWIn.theta.^2, 2)), [1, 1, T]), [3, 1, 2]);
    TWV_i_draws = sqrt( squeeze( sum(theta_draws .* (repmat(share, 1, 1, n_draw) .* output_TWIn.sigma_eps_i).^2, 2) ) );
    TWV_draws   = sqrt(TWV_c_draws.^2 + TWV_i_draws.^2);
    TWV_c       = quantile(TWV_c_draws, [signif, 0.5, 1-signif], 2);
    TWV_i       = quantile(TWV_i_draws, [signif, 0.5, 1-signif], 2);
    TWV         = quantile(TWV_draws,   [signif, 0.5, 1-signif], 2);
...
set(fig0, figsize{:}); 
print(fig0, [fig_archive_path 'FigE1_trend'], ['-d' fig_fmt])
...
% Tune and save figure
set(fig0, figsize{:}); 
print(fig0, [fig_archive_path 'FigE1_decomposition'], ['-d' fig_fmt])
