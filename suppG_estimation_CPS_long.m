...
% 
% The script estimates a flexible model in the long sample of the industry 
% data cut and produces figures with trend estimates and decompositions.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Upload estimation results
load([res_path 'results_' date_str '.mat'])

%%% FIGURES
% Define subsample for recent data
sample_line      = (year(dates) >= 1980);
line_ticks       = datetime((1983:5:2023), 1, 1);
...
set(fig0, figsize{:}); 
print(fig0, [fig_archive_path 'FigG1_trend'], ['-d' fig_fmt])
...
% Tune and save figure
set(fig0, figsize{:}); 
print(fig0, [fig_archive_path 'FigG1_decomposition'], ['-d' fig_fmt])

%%% COMPARISON WITH RECENT SAMPLE
% Upload results
full_obj  = load([res_path 'results_industries_long.mat']);
short_obj = load([res_path 'results_industries.mat']);
T_miss    = size(full_obj.TWIn, 1) - size(short_obj.TWIn, 1);

% Define normalizing constants for latent components
trend_c_norm_full  = mean(full_obj.TWIn_c(year(dates) <= 2019 & year(dates) >= 2017, 2));
trend_i_norm_full  = mean(full_obj.TWIn_i(year(dates) <= 2019 & year(dates) >= 2017, 2));
trend_c_norm_short = mean(short_obj.TWIn_c(year(dates(T_miss+1:end)) <= 2019 & year(dates(T_miss+1:end)) >= 2017, 2));
trend_i_norm_short = mean(short_obj.TWIn_i(year(dates(T_miss+1:end)) <= 2019 & year(dates(T_miss+1:end)) >= 2017, 2));

% Plot trend comparison with recession bands %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ylim_tmp = [0, 9];
fig0     = figure();
ax0      = axes();
fill(ax0, [recession_dates'; flipud(recession_dates')], repmat(kron(ylim_tmp, ones(1, 2))', 1, size(recession_dates, 1)), 0*white, 'facealpha', 0.1, 'linestyle', 'none')
hold('on')
plot0    = plot(ax0, dates, [infla_agg, full_obj.TWIn(:, 2), [NaN(T_miss, 1); short_obj.TWIn(:, 2)]]);
ylabel(ax0, '(percent)');
hold('off')
...
% Tune plot
legend(plot0, {'Wage growth (YoY)', 'TWIn (1983m1-2023m9)', 'TWIn (1997m1-2023m9)'}, 'location', 'best', font{:})
set(plot0, {'linewidth'}, {1.7; 3.5; 3.5})
set(plot0, {'color'}, {black; blue; red})
set(plot0, {'linestyle'}, {'-';  '-'; ':'})

% Tune and save figure
set(fig0, figsize{:}); 
print(fig0, [fig_archive_path 'FigG1_compare_trend'], ['-d' fig_fmt])

% Plot common trend comparison with recession bands %%%%%%%%%%%%%%%%%%%%%%%
ylim_tmp = [-3, 3];
fig0     = figure();
ax0      = axes();
fill(ax0, [recession_dates'; flipud(recession_dates')], repmat(kron(ylim_tmp, ones(1, 2))', 1, size(recession_dates, 1)), 0*white, 'facealpha', 0.1, 'linestyle', 'none')
hold('on')
plot0    = plot(ax0, dates, [full_obj.TWIn_c(:, 2) -  trend_c_norm_full, [NaN(T_miss, 1); short_obj.TWIn_c(:, 2) - trend_c_norm_short]]);
ylabel(ax0, 'Cumulative change from average over 2017-2019 (percent)');
hold('off')
...
set(ax0, font{:})
grid(ax0, 'off')
line(ax0, [dates(1), dates(end)], [0, 0], 'color', black, 'linestyle', '--')

% Tune plot
legend(plot0, {'Common trend (1983m1-2023m9)', 'Common trend (1997m1-2023m9)'}, 'location', 'best', font{:})
set(plot0, {'linewidth'}, {3.5; 3.5})
set(plot0, {'color'}, {blue; red})
set(plot0, {'linestyle'}, {'-'; ':'})

% Tune and save figure
set(fig0, figsize{:}); 
print(fig0, [fig_archive_path 'FigG1_compare_common'], ['-d' fig_fmt])

% Plot sector-specific trend comparison with recession bands %%%%%%%%%%%%%%
fig0     = figure();
ax0      = axes();
fill(ax0, [recession_dates'; flipud(recession_dates')], repmat(kron(ylim_tmp, ones(1, 2))', 1, size(recession_dates, 1)), 0*white, 'facealpha', 0.1, 'linestyle', 'none')
hold('on')
plot0    = plot(ax0, dates, [full_obj.TWIn_i(:, 2) - trend_i_norm_full, [NaN(T_miss, 1); short_obj.TWIn_i(:, 2) - trend_i_norm_short]]);
ylabel(ax0, 'Cumulative change from average over 2017-2019 (percent)');
hold('off')
...
% Tune plot
legend(plot0, {'Industry-specific trend (1983m1-2023m9)', 'Industry-specific trend (1997m1-2023m9)'}, 'location', 'best', font{:})
set(plot0, {'linewidth'}, {3.5; 3.5})
set(plot0, {'color'}, {blue; red})
set(plot0, {'linestyle'}, {'-'; ':'})
...
% Tune and save figure
set(fig0, figsize{:}); 
print(fig0, [fig_archive_path 'FigG1_compare_specific'], ['-d' fig_fmt])
